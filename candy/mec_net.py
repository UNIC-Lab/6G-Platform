...
def initialize_beta_L_matrices(num_requests):
    ...
    for row in range(2):
        for q in range(num_requests):
            L_matrix[row][q] = random.choice(mec_L_VALUES)
...
c = 0
d = 1
mec_dbo_best_fitness, mec_dbo_best_service_rate, mec_dbo_fitness_history, mec_dbo_service_rate_history = mec_DBO(population, population_size, num_iterations, c, d, user_requests, MEC_NETWORK_PARAMS)

# 绘制适应度历史
plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.plot(np.max(mec_dbo_fitness_history, axis=1), label='Max Fitness')
plt.plot(np.mean(mec_dbo_fitness_history, axis=1), label='Average Fitness')
...
# 绘制按需服务率历史
plt.subplot(1, 2, 2)
plt.plot(np.max(mec_dbo_service_rate_history, axis=1), label='Max Service Rate')
plt.plot(np.mean(mec_dbo_service_rate_history, axis=1), label='Average Service Rate')
...
plt.tight_layout()
plt.savefig('mec_dbo_3.pdf', bbox_inches='tight')
plt.savefig('mec_dbo_3.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
plt.show()
