...
for NUM_REQUESTS in num_requests_list:
    ...
    # 计算最大适应度和服务率历史
    max_dbo_fitness_histories[NUM_REQUESTS] = (np.max(all_avg_dbo_fitness_histories, axis=0)).tolist()
    max_dbo_service_rate_histories[NUM_REQUESTS] = (np.max(all_avg_dbo_service_rate_histories, axis=0)).tolist()
    max_ga_fitness_histories[NUM_REQUESTS] = (np.max(all_avg_ga_fitness_histories, axis=0)).tolist()
    max_ga_service_rate_histories[NUM_REQUESTS] = (np.max(all_avg_ga_service_rate_histories, axis=0)).tolist()
    max_greedy_fitness_histories[NUM_REQUESTS] = (np.max(all_avg_greedy_fitness_histories, axis=0)).tolist()
    max_greedy_service_rate_histories[NUM_REQUESTS] = (np.max(all_avg_greedy_service_rate_histories, axis=0)).tolist()
    max_random_fitness_histories[NUM_REQUESTS] = (np.max(all_avg_random_fitness_histories, axis=0)).tolist()
    max_random_service_rate_histories[NUM_REQUESTS] = (np.max(all_avg_random_service_rate_histories, axis=0)).tolist()
