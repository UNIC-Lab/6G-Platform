...
plt.subplot(1, 2, 1)
plt.plot(np.max(greedy_fitness_history, axis=1), label='Max Fitness')
plt.plot(np.mean(greedy_fitness_history, axis=1), label='Average Fitness')
...
# 绘制按需服务率历史
plt.subplot(1, 2, 2)
plt.plot(np.max(greedy_service_rate_history, axis=1), label='Max Service Rate')
plt.plot(np.mean(greedy_service_rate_history, axis=1), label='Average Service Rate')
...
# 保存图像
plt.tight_layout()
plt.savefig('greedy3.pdf', bbox_inches='tight')
plt.savefig('greedy3.png', bbox_inches='tight', dpi=600, pad_inches=0.0)

# 显示图像
plt.show()

# 打印最优个体信息
print("Best Individual's Fitness:", max(greedy_fitness_history[-1]))
print("Best Individual's Service Rate:", max(greedy_service_rate_history[-1]))
