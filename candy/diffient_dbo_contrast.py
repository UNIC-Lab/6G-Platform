...
# half滚球和跳舞行为
def half_ball_rolling_and_dancing_behavior(individual, GworstPosition, user_requests, NETWORK_PARAMS):
    new_individual = deepcopy(individual)
    b = 0.3  # 滚球行为的影响因子
    angle = np.random.randint(0, 360)  # 跳舞行为的角度
    theta = angle * np.pi / 180  # 转换为弧度
    ...
    for request_idx in range(NUM_REQUESTS):
        ...
        if sum_beta == 1:
            # 和为1时的特殊处理
            chosen_idx = random.choice([0, 1, 2])
            new_individual['beta_matrix'][chosen_idx][request_idx] = 0.5
            options = [0, 1, 2]
            options.remove(chosen_idx)
            other_idx = random.choice(options)
            new_individual['beta_matrix'][other_idx][request_idx] = 0.5
            new_snt_fitness_q = calculate_individual_q_fitness(new_individual, request_idx, user_requests[request_idx],
                                                               NETWORK_PARAMS)
            if new_snt_fitness_q < or_fitness_q:
                # 如果新适应度没有改善，则撤销更新
                new_individual['beta_matrix'][:, request_idx] = individual['beta_matrix'][:, request_idx]
            ...
            if random.random() < 0.25:
                if num_non_zero == 1:
                    ...
                    model_idx = non_zero_indices[0]
                    options = [0, 1, 2]
                    options.remove(model_idx)
                    other_idx = random.choice(options)
                    new_individual['beta_matrix'][model_idx][request_idx] = 0.5
                    new_individual['beta_matrix'][other_idx][request_idx] = 0.5
                elif num_non_zero == 2:
                    # 缝合AI模型部署
                    chosen_idx = random.choice(non_zero_indices)
                ...
                if num_non_zero == 1:
                    ...
                    options.remove(model_idx)
                    other_idx = random.choice(options)
                    attraction_factor = 0.01  # 这个值可以调整，以控制靠拢的速度
                    # 计算当前beta值与0.5之间的差距
                    beta_diff_model = 0.5 - individual['beta_matrix'][model_idx][request_idx]
                    beta_diff_other = 0.5 - individual['beta_matrix'][other_idx][request_idx]
                    # 更新beta值，使其向0.5靠拢
                    new_beta_value_model = individual['beta_matrix'][model_idx][request_idx] + attraction_factor * beta_diff_model
                    new_beta_value_other = individual['beta_matrix'][other_idx][request_idx] + attraction_factor * beta_diff_other
                    # 确保新的beta值在[0, 1]范围内
                    new_beta_value_model = max(0, min(new_beta_value_model, 1))
                    new_beta_value_other = max(0, min(new_beta_value_other, 1))
                    # 四舍五入到最接近的0.01，并更新个体
                    new_individual['beta_matrix'][model_idx][request_idx] = round(new_beta_value_model, 2)
                    new_individual['beta_matrix'][other_idx][request_idx] = round(new_beta_value_other, 2)
                # 如果有两个元素大于0
                if num_non_zero == 2:
                    for model_idx in non_zero_indices:
                        # 定义向0.5靠拢的吸引因子
                        attraction_factor = 0.01  # 这个值可以调整，以控制靠拢的速度
                        # 计算当前beta值与0.5之间的差距
                        beta_diff = 0.5 - individual['beta_matrix'][model_idx][request_idx]
                        # 更新beta值，使其向0.5靠拢
                        new_beta_value = individual['beta_matrix'][model_idx][request_idx] + attraction_factor * beta_diff
                        # 确保新的beta值在[0, 1]范围内
                        new_beta_value = max(0, min(new_beta_value, 1))
                        # 四舍五入到最接近的0.01，并更新个体
                        new_individual['beta_matrix'][model_idx][request_idx] = round(new_beta_value, 2)
        #对L矩阵的更新优化
        L_values = random.sample([1, 2, 3, 4], 2)
        new_individual['L_matrix'][0][request_idx], new_individual['L_matrix'][1][request_idx] = sorted(L_values)
...
# single滚球和跳舞行为
def single_ball_rolling_and_dancing_behavior(individual, GworstPosition, user_requests, NETWORK_PARAMS):
    new_individual = deepcopy(individual)
    b = 0.3  # 滚球行为的影响因子
    angle = np.random.randint(0, 360)  # 跳舞行为的角度
    theta = angle * np.pi / 180  # 转换为弧度
    ...
    for request_idx in range(NUM_REQUESTS):
        ...
        if sum_beta == 1:
            ...
            new_individual['beta_matrix'][:, request_idx] = 0
            new_individual['beta_matrix'][chosen_idx][request_idx] = 1
            new_sin_fitness_q = calculate_individual_q_fitness(new_individual, request_idx, user_requests[request_idx],
                                                               NETWORK_PARAMS)
            if new_sin_fitness_q < or_fitness_q:
                # 如果新适应度没有改善，则撤销更新
                new_individual['beta_matrix'][:, request_idx] = individual['beta_matrix'][:, request_idx]
            ...
            if random.random() < 0.95:
                ...
                if num_non_zero == 2:
                    ...
                    if np.random.rand() < 0.9:  # 滚球行为的概率
                        ...
                        new_value = individual['beta_matrix'][chosen_idx][request_idx] + change
                        new_value = max(0, min(new_value, 1))  # 确保在有效范围内
                        new_individual['beta_matrix'][chosen_idx][request_idx] = round(new_value, 2)  # 更新后的值四舍五入到最接近的0.01
                    else:  # 跳舞行为
                        if angle not in [0, 90, 180, 270]:  # 特定角度时保持位置不变
                            change = np.tan(theta) * (GworstPosition['beta_matrix'][chosen_idx][request_idx] -
                                                      individual['beta_matrix'][chosen_idx][request_idx])
...
# single觅食行为
def single_foraging_behavior(individual, user_requests, NETWORK_PARAMS, Lbb, Ubb):
    new_individual = deepcopy(individual)
        ...
        if sum_beta == 1:
            ...
            if random.random() < 0.8:
                if num_non_zero == 1:
                    ...
                    chosen_idx = random.choice(non_zero_indices)
                    new_individual['beta_matrix'][:, request_idx] = 0
                    new_individual['beta_matrix'][chosen_idx][request_idx] = 1.0
                ...
                # 如果有两个元素大于0
                    ...
                    chosen_idx = random.choice(non_zero_indices)
                    change = (np.random.random()) * (individual['beta_matrix'][chosen_idx][request_idx] - Lbb['beta_matrix'][chosen_idx][
                            request_idx]) + (np.random.random()) * (individual['beta_matrix'][chosen_idx][request_idx] -
                                                        Ubb['beta_matrix'][chosen_idx][request_idx])
                    change = round(change, 2)
                    new_value = individual['beta_matrix'][chosen_idx][request_idx] + change
                    new_value = max(0, min(new_value, 1))
                    new_individual['beta_matrix'][chosen_idx][request_idx] = new_value
...
# single偷窃行为
def single_stealing_behavior(individual, GbestPosition, GbestPosition_local, user_requests, NETWORK_PARAMS):
    new_individual = deepcopy(individual)
    s = 0.5  # 偷窃行为的影响因子
...
# single繁殖行为
def single_egg_laying_behavior_with_uniform_mutation(individual, user_requests, NETWORK_PARAMS, GbestPosition_local, Lbstar, Ubstar):
    new_individual = deepcopy(individual)
        ...
        if sum_beta == 1:
            ...
            if random.random() < 0.8:
                ...
                # 如果有两个元素大于0
                    ...
                    chosen_idx = random.choice(non_zero_indices)
                    new_value = GbestPosition_local['beta_matrix'][chosen_idx][request_idx] + (np.random.random()) * (
                            individual['beta_matrix'][chosen_idx][request_idx] - Lbstar['beta_matrix'][chosen_idx][
                        request_idx]) + (np.random.random()) * (individual['beta_matrix'][chosen_idx][request_idx] -
                                                           Ubstar['beta_matrix'][chosen_idx][request_idx])
                    new_value = round(new_value, 2)
                    new_value = max(0, min(new_value, 1))
                    new_individual['beta_matrix'][chosen_idx][request_idx] = new_value
...
def half_DBO(population, population_size, num_iterations, c, d, user_requests, NETWORK_PARAMS):
    """
    ...
    ub = d
    # 蜣螂算法的适应度和按需服务率
    half_dbo_fitness_history = []
    half_dbo_service_rate_history = []
    ...
    # 记录初始适应度和按需服务率
    half_dbo_fitness_history.append(dbo_ori_fitnesses)
    half_dbo_service_rate_history.append(dbo_ori_service_rates)
    ...
    for t in range(num_iterations):
        ...
        for i in range(population_size):
            ...
            new_individual = half_ball_rolling_and_dancing_behavior(individual, GworstPosition, user_requests, NETWORK_PARAMS)
        ...
        # 记录适应度和按需服务率
        half_dbo_fitness_history.append(dbo_fitnesses)
        half_dbo_service_rate_history.append(dbo_service_rates)
        print(f"dbo完成迭代 {t + 1}，总耗时 {time.time() - iteration_start_time:.2f} 秒。\n")
    # 在迭代结束后存储蜣螂算法的最终结果
    dbo_best_fitness_idx = np.argmax(half_dbo_fitness_history[-1])
    half_dbo_best_individual = population[dbo_best_fitness_idx]
    half_dbo_best_fitness = max(half_dbo_fitness_history[-1])
    half_dbo_best_service_rate = max(half_dbo_service_rate_history[-1])

    return half_dbo_best_fitness, half_dbo_best_service_rate, half_dbo_fitness_history, half_dbo_service_rate_history

def single_DBO(population, population_size, num_iterations, c, d, user_requests, NETWORK_PARAMS):
    """
    :param population_size: 种群数量
    :param num_iterations: 迭代次数
    :param c: 迭代范围下界
    ...
    for t in range(num_iterations):
        ...
        for i in range(pNum):
            individual = population[i]
            dbo_old_individual_fitness, _ = dbo_calculate_individual_fitness(individual, user_requests, NETWORK_PARAMS)
            new_individual = single_ball_rolling_and_dancing_behavior(individual, GworstPosition, user_requests, NETWORK_PARAMS)
            dbo_new_individual_fitness, _ = dbo_calculate_individual_fitness(new_individual, user_requests, NETWORK_PARAMS)
            if dbo_new_individual_fitness >= dbo_old_individual_fitness:
                new_population[i] = new_individual
            else:
        ...
        for i in range(pNum+1, 120):      # Equation(4)
            individual = population[i]
            dbo_old_individual_fitness, _ = dbo_calculate_individual_fitness(individual, user_requests, NETWORK_PARAMS)
            new_individual = single_egg_laying_behavior_with_uniform_mutation(individual, user_requests, NETWORK_PARAMS, GbestPosition_local,Lbstar, Ubstar)
            dbo_new_individual_fitness, _ = dbo_calculate_individual_fitness(new_individual, user_requests, NETWORK_PARAMS)
            if dbo_new_individual_fitness >= dbo_old_individual_fitness:
                new_population[i] = new_individual
            else:
        ...
        for i in range(121, 190):           # Equation(6)
            individual = population[i]
            dbo_old_individual_fitness, _ = dbo_calculate_individual_fitness(individual, user_requests, NETWORK_PARAMS)
            new_individual = single_foraging_behavior(individual, user_requests, NETWORK_PARAMS, Lbb, Ubb)
            dbo_new_individual_fitness, _ = dbo_calculate_individual_fitness(new_individual, user_requests, NETWORK_PARAMS)
            if dbo_new_individual_fitness >= dbo_old_individual_fitness:
                new_population[i] = new_individual
            else:
        ...
        for i in range(191, population_size):           # Equation(7)
            individual = population[i]
            dbo_old_individual_fitness, _ = dbo_calculate_individual_fitness(individual, user_requests, NETWORK_PARAMS)
            new_individual = single_stealing_behavior(individual, GbestPosition, GbestPosition_local, user_requests, NETWORK_PARAMS)
            dbo_new_individual_fitness, _ = dbo_calculate_individual_fitness(new_individual, user_requests, NETWORK_PARAMS)
            if dbo_new_individual_fitness >= dbo_old_individual_fitness:
                new_population[i] = new_individual
            else:
...
avg_single_dbo_fitness_histories = {num_requests: [] for num_requests in num_requests_list}
avg_single_dbo_service_rate_histories = {num_requests: [] for num_requests in num_requests_list}
avg_half_dbo_fitness_histories = {num_requests: [] for num_requests in num_requests_list}
avg_half_dbo_service_rate_histories = {num_requests: [] for num_requests in num_requests_list}
...
max_single_dbo_fitness_histories = {num_requests: [] for num_requests in num_requests_list}
max_single_dbo_service_rate_histories = {num_requests: [] for num_requests in num_requests_list}
max_half_dbo_fitness_histories = {num_requests: [] for num_requests in num_requests_list}
max_half_dbo_service_rate_histories = {num_requests: [] for num_requests in num_requests_list}
...
for NUM_REQUESTS in num_requests_list:
    ...
    all_avg_single_dbo_fitness_histories = []
    all_avg_single_dbo_service_rate_histories = []
    all_avg_half_dbo_fitness_histories = []
    all_avg_half_dbo_service_rate_histories = []
    # 初始化累计max适应度和服务率历史
    all_max_dbo_fitness_histories = []
    all_max_dbo_service_rate_histories = []
    all_max_single_dbo_fitness_histories = []
    all_max_single_dbo_service_rate_histories = []
    all_max_half_dbo_fitness_histories = []
    all_max_half_dbo_service_rate_histories = []
    for run in range(num_runs):
        ...
        single_dbo_best_fitness, single_dbo_best_service_rate, single_dbo_fitness_hist, single_dbo_service_rate_hist = single_DBO(population, population_size, num_iterations, c, d, user_requests, NETWORK_PARAMS)
        half_dbo_best_fitness, half_dbo_best_service_rate, half_dbo_fitness_hist, half_dbo_service_rate_hist = half_DBO(population, population_size, num_iterations, c, d, user_requests, NETWORK_PARAMS)
        save_as_json(dbo_fitness_hist, f'diffient_{NUM_REQUESTS}_dbo_fitness_hist_{run}.json')
        save_as_json(dbo_service_rate_hist, f'diffient_{NUM_REQUESTS}_dbo_service_rate_hist_{run}.json')
        save_as_json(single_dbo_fitness_hist, f'diffient_{NUM_REQUESTS}_single_dbo_fitness_hist_{run}.json')
        save_as_json(single_dbo_service_rate_hist, f'diffient_{NUM_REQUESTS}_single_dbo_service_rate_hist_{run}.json')
        save_as_json(half_dbo_fitness_hist, f'diffient_{NUM_REQUESTS}_half_dbo_fitness_hist_{run}.json')
        save_as_json(half_dbo_service_rate_hist, f'diffient_{NUM_REQUESTS}_half_dbo_service_rate_hist_{run}.json')
        # 保存avg结果
        ...
        all_avg_single_dbo_fitness_histories.append(np.mean(single_dbo_fitness_hist, axis = 1))
        all_avg_single_dbo_service_rate_histories.append(np.mean(single_dbo_service_rate_hist, axis = 1))
        all_avg_half_dbo_fitness_histories.append(np.mean(half_dbo_fitness_hist, axis = 1))
        all_avg_half_dbo_service_rate_histories.append(np.mean(half_dbo_service_rate_hist, axis = 1))
        #保存max
        ...
        all_max_single_dbo_fitness_histories.append(np.max(single_dbo_fitness_hist, axis=1))
        all_max_single_dbo_service_rate_histories.append(np.max(single_dbo_service_rate_hist, axis=1))
        all_max_half_dbo_fitness_histories.append(np.max(half_dbo_fitness_hist, axis=1))
        all_max_half_dbo_service_rate_histories.append(np.max(half_dbo_service_rate_hist, axis=1))
    # 计算平均适应度和服务率历史
    ...
    avg_single_dbo_fitness_histories[NUM_REQUESTS] = (np.mean(all_avg_single_dbo_fitness_histories, axis=0)).tolist()
    avg_single_dbo_service_rate_histories[NUM_REQUESTS] = (np.mean(all_avg_single_dbo_service_rate_histories, axis=0)).tolist()
    avg_half_dbo_fitness_histories[NUM_REQUESTS] = (np.mean(all_avg_half_dbo_fitness_histories, axis=0)).tolist()
    avg_half_dbo_service_rate_histories[NUM_REQUESTS] = (np.mean(all_avg_half_dbo_service_rate_histories, axis=0)).tolist()
    save_as_json(avg_dbo_fitness_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_avg_dbo_fitness_histories.json')
    save_as_json(avg_dbo_service_rate_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_avg_dbo_service_rate_histories.json')
    save_as_json(avg_single_dbo_fitness_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_avg_single_dbo_fitness_histories.json')
    save_as_json(avg_single_dbo_service_rate_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_avg_single_dbo_service_rate_histories.json')
    save_as_json(avg_half_dbo_fitness_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_avg_half_dbo_fitness_histories.json')
    save_as_json(avg_half_dbo_service_rate_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_avg_half_dbo_service_rate_histories.json')
    # 计算最大适应度和服务率历史
    ...
    max_single_dbo_fitness_histories[NUM_REQUESTS] = (np.max(all_max_single_dbo_fitness_histories, axis=0)).tolist()
    max_single_dbo_service_rate_histories[NUM_REQUESTS] = (np.max(all_max_single_dbo_service_rate_histories, axis=0)).tolist()
    max_half_dbo_fitness_histories[NUM_REQUESTS] = (np.max(all_max_half_dbo_fitness_histories, axis=0)).tolist()
    max_half_dbo_service_rate_histories[NUM_REQUESTS] = (np.max(all_max_half_dbo_service_rate_histories, axis=0)).tolist()
    save_as_json(max_dbo_fitness_histories[NUM_REQUESTS], f'diffient_{NUM_REQUESTS}_max_dbo_fitness_histories.json')
    save_as_json(max_dbo_service_rate_histories[NUM_REQUESTS],f'diffient_{NUM_REQUESTS}_max_dbo_service_rate_histories.json')
    save_as_json(max_single_dbo_fitness_histories[NUM_REQUESTS],f'diffient_{NUM_REQUESTS}_max_single_dbo_fitness_histories.json')
    save_as_json(max_single_dbo_service_rate_histories[NUM_REQUESTS],f'diffient_{NUM_REQUESTS}_max_single_dbo_service_rate_histories.json')
    save_as_json(max_half_dbo_fitness_histories[NUM_REQUESTS],f'diffient_{NUM_REQUESTS}_max_half_dbo_fitness_histories.json')
    save_as_json(max_half_dbo_service_rate_histories[NUM_REQUESTS],f'diffient_{NUM_REQUESTS}_max_half_dbo_service_rate_histories.json')
...
final_max_sin_dbo_fitness = [np.max(max_single_dbo_fitness_histories[nr][-1]) for nr in num_requests_list]
final_max_sin_dbo_service_rate = [np.max(max_single_dbo_service_rate_histories[nr][-1]) for nr in num_requests_list]
final_max_half_dbo_fitness = [np.max(max_half_dbo_fitness_histories[nr][-1]) for nr in num_requests_list]
final_max_half_dbo_service_rate = [np.max(max_half_dbo_service_rate_histories[nr][-1]) for nr in num_requests_list]

#保存文件到本地
save_as_json(avg_dbo_fitness_histories, 'diffient_avg_dbo_fitness_histories.json')
save_as_json(avg_dbo_service_rate_histories, 'diffient_avg_dbo_service_rate_histories.json')
save_as_json(avg_single_dbo_fitness_histories, 'diffient_avg_single_dbo_fitness_histories.json')
save_as_json(avg_single_dbo_service_rate_histories, 'diffient_avg_single_dbo_service_rate_histories.json')
save_as_json(avg_half_dbo_fitness_histories, 'diffient_avg_half_dbo_fitness_histories.json')
save_as_json(avg_half_dbo_service_rate_histories, 'diffient_avg_half_dbo_service_rate_histories.json')
save_as_json(max_dbo_fitness_histories, 'diffient_max_dbo_fitness_histories.json')
save_as_json(max_dbo_service_rate_histories, 'diffient_max_dbo_service_rate_histories.json')
save_as_json(max_single_dbo_fitness_histories, 'diffient_max_single_dbo_fitness_histories.json')
save_as_json(max_single_dbo_service_rate_histories, 'diffient_max_single_dbo_service_rate_histories.json')
save_as_json(max_half_dbo_fitness_histories, 'diffient_max_half_dbo_fitness_histories.json')
save_as_json(max_half_dbo_service_rate_histories, 'diffient_max_half_dbo_service_rate_histories.json')
save_as_json(final_max_dbo_fitness, 'diffient_final_max_dbo_fitness.json')
save_as_json(final_max_dbo_service_rate, 'diffient_final_max_dbo_service_rate.json')
save_as_json(final_max_sin_dbo_fitness, 'diffient_final_max_sin_dbo_fitness.json')
save_as_json(final_max_sin_dbo_service_rate, 'diffient_final_max_sin_dbo_service_rate.json')
save_as_json(final_max_half_dbo_fitness, 'diffient_final_max_half_dbo_fitness.json')
save_as_json(final_max_half_dbo_service_rate, 'diffient_final_max_half_dbo_service_rate.json')
...
for num_requests in num_requests_list:
    ...
    plt.plot(range(num_iterations+1), avg_single_dbo_fitness_histories[num_requests], label='Single_DBO', color="#FABC60", linewidth=1)
    plt.plot(range(num_iterations+1), avg_half_dbo_fitness_histories[num_requests], label='Fix_DBO', color="#3A9679", linewidth=1)
    ...
    plt.savefig(f'dbo+sin+fix_avg_fitness_history_{num_requests}_requests.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_avg_fitness_history_{num_requests}_requests.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.plot(range(num_iterations+1), avg_single_dbo_service_rate_histories[num_requests], label='Single_DBO', color="#FABC60", linewidth=1)
    plt.plot(range(num_iterations+1), avg_half_dbo_service_rate_histories[num_requests], label='Fix_DBO', color="#3A9679", linewidth=1)
    # 设置 X 轴和 Y 轴的主刻度标签只显示整数
    plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    plt.xlabel('Iteration')
    plt.ylabel('On-demand Service Rate')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_Service Rate_history_{num_requests}_requests.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_Service Rate_history_{num_requests}_requests.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.plot(range(num_iterations+1), max_single_dbo_fitness_histories[num_requests], label='Single_DBO', color="#FABC60", linewidth=1)
    plt.plot(range(num_iterations+1), max_half_dbo_fitness_histories[num_requests], label='Fix_DBO', color="#3A9679", linewidth=1)
    # 设置 X 轴和 Y 轴的主刻度标签只显示整数
    plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    plt.gca().yaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    plt.xlabel('Iteration')
    plt.ylabel('Satisfaction')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_max_fitness_history_{num_requests}_requests.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_max_fitness_history_{num_requests}_requests.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.plot(range(num_iterations+1), max_single_dbo_service_rate_histories[num_requests], label='Single_DBO', color="#FABC60", linewidth=1)
    plt.plot(range(num_iterations+1), max_half_dbo_service_rate_histories[num_requests], label='Fix_DBO', color="#3A9679", linewidth=1)
    # 设置 X 轴和 Y 轴的主刻度标签只显示整数
    plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
    plt.xlabel('Iteration')
    plt.ylabel('On-demand Service Rate')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_Max_Service Rate_history_{num_requests}_requests.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_Max_Service Rate_history_{num_requests}_requests.png', bbox_inches='tight', dpi=600, pad_inches=0.0)

# 绘制NUM_MAX适应度图
plt.figure(figsize=(7, 6))
plt.plot(num_requests_list, final_max_dbo_fitness, 'o-', label='DBO', color="#E16262", linewidth=2)
plt.plot(num_requests_list, final_max_sin_dbo_fitness, 's-', label='Single_DBO', color="#FABC60", linewidth=2)
plt.plot(num_requests_list, final_max_half_dbo_fitness, '^-', label='Fix_DBO', color="#3A9679", linewidth=2)
# 设置 X 轴和 Y 轴的主刻度标签只显示整数
plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
plt.gca().yaxis.set_major_locator(ticker.MaxNLocator(integer=True))
...
plt.legend(loc="lower right")
plt.grid()
plt.savefig('Max Fitness for Different Number of Requests.pdf', bbox_inches='tight')
plt.savefig('Max Fitness for Different Number of Requests.png', bbox_inches='tight', dpi=600, pad_inches=0.0)

# 绘制NUM_MAX服务率图
plt.figure(figsize=(7, 6))
plt.plot(num_requests_list, final_max_dbo_service_rate, 'o-', label='DBO', color="#E16262", linewidth=2)
plt.plot(num_requests_list, final_max_sin_dbo_service_rate, 's-', label='Single_DBO', color="#FABC60", linewidth=2)
plt.plot(num_requests_list, final_max_half_dbo_service_rate, '^-', label='Fix_DBO', color="#3A9679", linewidth=2)
# 设置 X 轴和 Y 轴的主刻度标签只显示整数
plt.gca().xaxis.set_major_locator(ticker.MaxNLocator(integer=True))
plt.xlabel('Number of Requests')
plt.ylabel('On-demand Service Rate')
plt.legend(loc="lower right")
plt.grid()
plt.savefig('Max Service Rate for Different Number of Requests.pdf', bbox_inches='tight')
plt.savefig('Max Service Rate for Different Number of Requests.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.ylabel('用户满意度')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_avg_fitness_history_{num_requests}_requests_chinese.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_avg_fitness_history_{num_requests}_requests_chinese.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.ylabel('按需服务率')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_Service Rate_history_{num_requests}_requests_chinese.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_Service Rate_history_{num_requests}_requests_chinese.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.ylabel('用户满意度')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_max_fitness_history_{num_requests}_requests_chinese.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_max_fitness_history_{num_requests}_requests_chinese.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
for num_requests in num_requests_list:
    ...
    plt.ylabel('按需服务率')
    plt.legend(loc="lower right")
    plt.savefig(f'dbo+sin+fix_Max_Service Rate_history_{num_requests}_requests_chinese.pdf', bbox_inches='tight')
    plt.savefig(f'dbo+sin+fix_Max_Service Rate_history_{num_requests}_requests_chinese.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
plt.legend(loc="lower right")
plt.grid()
plt.savefig('Max Fitness for Different Number of Requests_chinese.pdf', bbox_inches='tight')
plt.savefig('Max Fitness for Different Number of Requests_chinese.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
...
plt.legend(loc="lower right")
plt.grid()
plt.savefig('Max Service Rate for Different Number of Requests_chinese.pdf', bbox_inches='tight')
plt.savefig('Max Service Rate for Different Number of Requests_chinese.png', bbox_inches='tight', dpi=600, pad_inches=0.0)