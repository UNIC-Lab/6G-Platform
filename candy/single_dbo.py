...
c = 0
d = 1
single_dbo_best_fitness, single_dbo_best_service_rate, single_dbo_fitness_history, single_dbo_service_rate_history = single_DBO(population, population_size, num_iterations, c, d, user_requests, NETWORK_PARAMS)

# 绘制适应度历史
plt.figure(figsize=(10, 4))
plt.subplot(1, 2, 1)
plt.plot(np.max(single_dbo_fitness_history, axis=1), label='Max Fitness')
plt.plot(np.mean(single_dbo_fitness_history, axis=1), label='Average Fitness')
...
# 绘制按需服务率历史
plt.subplot(1, 2, 2)
plt.plot(np.max(single_dbo_service_rate_history, axis=1), label='Max Service Rate')
plt.plot(np.mean(single_dbo_service_rate_history, axis=1), label='Average Service Rate')
...
plt.tight_layout()
plt.savefig('single_dbo_3.pdf', bbox_inches='tight')
plt.savefig('single_dbo_3.png', bbox_inches='tight', dpi=600, pad_inches=0.0)
plt.show()
